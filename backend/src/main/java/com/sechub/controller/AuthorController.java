...
public class AuthorController {
    ...
    @PostMapping("/paths/{id}/publish") public ResponseEntity<ApiResponse<Void>> publishPath(@PathVariable UUID id,@AuthenticationPrincipal UserDetails u){service.publishPath(u.getUsername(),id);return ResponseEntity.ok(ApiResponse.success("Đã xuất bản lộ trình",null));}
    ...
    @PostMapping("/labs/{id}/publish") public ResponseEntity<ApiResponse<Void>> publishLab(@PathVariable UUID id,@AuthenticationPrincipal UserDetails u){service.publishLab(u.getUsername(),id);return ResponseEntity.ok(ApiResponse.success("Đã xuất bản challenge",null));}
    @DeleteMapping("/paths/{id}") public ResponseEntity<ApiResponse<Void>> deletePath(@PathVariable UUID id,@AuthenticationPrincipal UserDetails u){service.deletePath(u.getUsername(),id);return ResponseEntity.ok(ApiResponse.success("Đã xoá bản nháp",null));}
    @DeleteMapping("/labs/{id}") public ResponseEntity<ApiResponse<Void>> deleteLab(@PathVariable UUID id,@AuthenticationPrincipal UserDetails u){service.deleteLab(u.getUsername(),id);return ResponseEntity.ok(ApiResponse.success("Đã xoá challenge nháp",null));}
}
